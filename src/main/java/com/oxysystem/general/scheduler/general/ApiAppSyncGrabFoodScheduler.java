...
@Component
@ConditionalOnProperty(value = "schedule.grab-food-menus-sync.enabled", havingValue = "true")
public class ApiAppSyncGrabFoodScheduler {
    private static final Logger LOGGER = LoggerFactory.getLogger(ApiAppSyncGrabFoodScheduler.class);
    ...
    @Scheduled(cron = "${schedule.grab-food-menus-sync.cron}")
    ...
    private Mono<Void> executeWithTokenRetry(BatchUpdateMenuRequestDTO request,
        ...
        return grabFoodMenuSyncService.batchUpdateMenuReactive(request, token)
                .doOnSuccess(v -> {
                ...
                .onErrorResume(err -> {
                    if (isUnauthorized(err)) {
                        ...
                        return getGrabTokenMono().flatMap(newToken ->
                                grabFoodMenuSyncService.batchUpdateMenuReactive(request, newToken)
                                        .doOnSuccess(v2 -> {
                                            List<Long> ids = merchantToApiSyncIds.getOrDefault(request.getMerchantID(), Collections.emptyList());
                                            apiSyncIdsSuccess.addAll(ids);
                                        })
    ...
    private Mono<String> getGrabTokenMono() {
        ...
        synchronized (tokenLock) {
            ...
            this.inFlightTokenMono = Mono.fromCallable(grabFoodOAuthService::getAccessToken)
                    .subscribeOn(Schedulers.boundedElastic())
