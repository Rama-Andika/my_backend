...
@Component
@ConditionalOnProperty(value = "schedule.grab-food-update-menus-notification.enabled", havingValue = "true")
public class MenuSyncFoodScheduler {

    private static final Logger LOGGER = LoggerFactory.getLogger(MenuSyncFoodScheduler.class);

    private final GrabFoodMenuSyncServiceImpl grabMenuSyncService;
    private final GrabFoodOAuthServiceImpl grabOAuthService;
    private final StrukKasirService strukKasirService;
    private final TenantRepository tenantRepository;
    private final RateLimiter rateLimiter;

    public MenuSyncFoodScheduler(GrabFoodMenuSyncServiceImpl grabMenuSyncService,
                                 GrabFoodOAuthServiceImpl grabOAuthService,
                                 StrukKasirService strukKasirService,
                                 TenantRepository tenantRepository,
                                 @Qualifier("menuNotifyScheduler") Scheduler scheduler) {
        this.grabMenuSyncService = grabMenuSyncService;
        this.grabOAuthService = grabOAuthService;
        this.strukKasirService = strukKasirService;
        this.tenantRepository = tenantRepository;
        this.rateLimiter = new RateLimiter(scheduler);
    }

    @Scheduled(cron = "${schedule.grab-food-update-menus-notification.cron}")
    public void updateAllMenuNotification() {
        Mono<String> tokenMono = Mono.fromCallable(grabOAuthService::getGrabToken)
        ...
        Mono<List<String>> merchantIdsMono = Mono.fromCallable(() -> {
                    ...
                    for (Tenant tenant : tenants) {
                        try {
                            ...
                            LOGGER.error("Gagal menarik data merchant GrabFood untuk tenant {}: {}", tenant.getTenantId(), e.getMessage());
                        } finally {
                            TenantContext.clear();
                        }
                    }
        ...
        Mono.zip(tokenMono, merchantIdsMono)
                .flatMapMany(tuple -> {
                    ...
                    return rateLimiter.process(merchantIds, token, grabMenuSyncService);
                })
    ...
    static class RateLimiter {
        ...
        public Flux<Void> process(List<String> merchantIds, String token, GrabFoodMenuSyncServiceImpl grabMenuSyncService) {
            if (merchantIds.isEmpty()) {
                return Flux.empty();
            }
            ...
            return Flux.fromIterable(merchantIds)
                    .flatMap(id -> tryProcessWithPermit(id, token, grabMenuSyncService, counter), 10);
        }

        private Mono<Void> tryProcessWithPermit(String id, String token, GrabFoodMenuSyncServiceImpl grabMenuSyncService, AtomicInteger counter) {
            return Mono.defer(() -> {
                if (semaphore.tryAcquire()) {
                    ...
                    return grabMenuSyncService.updateMenuNotifyReactive(token, new UpdateMenuNotificationRequestDTO(id))
                            .doFinally(signal -> {
                                semaphore.release();
                    ...
                    return Mono.delay(Duration.ofMillis(100), scheduler)
                            .then(tryProcessWithPermit(id, token, grabMenuSyncService, counter));
                }
            });
        }
