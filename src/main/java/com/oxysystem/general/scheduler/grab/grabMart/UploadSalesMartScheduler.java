...
@Component
@ConditionalOnProperty(value = "schedule.grab-mart-upload-sales.enabled", havingValue = "true")
public class UploadSalesMartScheduler {

    private static final Logger LOGGER = LoggerFactory.getLogger(UploadSalesMartScheduler.class);

    private final GrabMartOrderSyncServiceImpl orderSyncService;
    private final GrabMartOrderSyncPartnerServiceImpl orderSyncPartnerService;
    ...
    public UploadSalesMartScheduler(GrabMartOrderSyncServiceImpl orderSyncService, GrabMartOrderSyncPartnerServiceImpl orderSyncPartnerService, SalesService salesService, GrabMartOAuthServiceImpl oAuthService, StrukKasirService strukKasirService, StockService stockService, TenantRepository tenantRepository, @Qualifier("uploadSalesGrabMartScheduler") Scheduler scheduler) {
    ...
    @Scheduled(cron = "${schedule.grab-mart-upload-sales.cron}")
    public void uploadSales() {
        Mono<String> tokenMono = oAuthService.getGrabTokenReactive()
                .doOnNext(token -> LOGGER.info("Grabmart token fetched length={}", token.length()));
        ...
        Mono<List<String>> merchantIdsMono = Mono.fromCallable(() -> {
                    ...
                    for (Tenant tenant : tenants) {
                        try {
                            ...
                            LOGGER.error("Failed to fetch GrabMart merchants for sales upload for tenant {}: {}", tenant.getTenantId(), e.getMessage());
                        } finally {
                            TenantContext.clear();
                        }
                    }
        ...
        Mono.zip(tokenMono, merchantIdsMono)
                .flatMapMany(tuple -> {
                    ...
                    if (token.isEmpty()) {
                        LOGGER.error("Grabmart token is empty, aborting upload sales pipeline");
                        return Flux.empty();
                    }
    ...
    static class RateLimiter {
        ...
        public Flux<Void> process(List<String> merchantIds,
                                  String token,
                                  GrabMartOrderSyncServiceImpl orderSyncService,
                                  GrabMartOrderSyncPartnerServiceImpl orderSyncPartnerService,
        ...
        private Mono<Void> processOrders(String merchantId,
            ...
            if(!orderIdsCanceled.isEmpty()) {
                stockService.deleteStocksByNoFakturs(new ArrayList<>(orderIdsCanceled));
                LOGGER.info("Success delete Orders canceled for merchant {}", merchantId);
            }
