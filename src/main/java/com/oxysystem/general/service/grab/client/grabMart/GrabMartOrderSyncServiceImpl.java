...
@Service
@Slf4j
public class GrabMartOrderSyncServiceImpl implements GrabOrderSyncService {
    ...
    public GrabMartOrderSyncServiceImpl(WebClient grabMartClient, LocationService locationService, GrabMartOAuthServiceImpl grabMartOAuthService, StrukKasirService strukKasirService, ItemMasterService itemMasterService, SalesService salesService, GrabOrderSyncCommon grabOrderSyncCommon) {
    ...
    public ResponseEntity<?> listOrder(String token, String merchantID, String date, Integer page, List<String> orderIds) {
        return grabOrderSyncCommon.listOrder(grabMartClient, token, merchantID, date, page, orderIds);
    }

    @Override
    public ListOrderResponseDTO getListOrder(String token, String merchantID, String date, Integer page, List<String> orderIDs) {
        return grabOrderSyncCommon.getListOrder(grabMartClient, token, merchantID, date, page, orderIDs);
    }
    ...
    public ResponseEntity<?> listOrderMappingToListOrderDTO(Long locationId, String date, Integer page, List<String> orderIDs) {
        ...
        if(strukKasir.getGrabMerchantId() == null || strukKasir.getGrabMerchantId().isEmpty()) throw new ResourceNotFoundException("merchant ID cannot be empty!");

        String merchantID = strukKasir.getGrabMerchantId();

        ListOrderResponseDTO listOrderResponseDTO = grabOrderSyncCommon.getListOrder(grabMartClient, token, merchantID, date, page, orderIDs);
    ...
    public Mono<ListOrderResponseDTO> getListOrderReactive(String token, String merchantID, String date, Integer page, List<String> orderIDs) {
        return grabOrderSyncCommon.getListOrderReactive(grabMartClient, token, merchantID, date, page, orderIDs);
    }
}
