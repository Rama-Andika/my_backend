...
@Service
@Slf4j
public class GrabFoodMenuSyncPartnerServiceImpl implements GrabMenuSyncPartnerService {
    ...
    public GrabFoodMenuSyncPartnerServiceImpl(StrukKasirService strukKasirService, LocationService locationService, CurrencyService currencyService, GrabMenuSyncStatusService grabMenuSyncStatusService, GrabfoodItemMappingService grabfoodItemMappingService, SystemMainService systemMainService, ItemMasterService itemMasterService) {
    ...
    public ResponseEntity<?> getMenu(String merchantID, String partnerMerchantID) {
        ...
        StrukKasir strukKasir = strukKasirService.findByGrabFoodMerchantId(merchantID).orElseThrow(() -> new ResourceNotFoundException("merchant ID not found"));
    ...
    private List<CategoryDTO> getCategoryDTOS(Location location, CurrencyDTO currency) {
        SystemMain systemMainGrabFoodGolPrice = systemMainService.findSystemPropertyName("GRABFOOD_GOL_PRICE").orElse(null);
        String golPrice = systemMainGrabFoodGolPrice != null ? systemMainGrabFoodGolPrice.getValueprop() : location.getGolPrice();

        List<GrabfoodItemMappingViewDTO> grabfoodItemMappingViewDTOS = grabfoodItemMappingService.getListGrabfoodItemMapping(golPrice, location.getLocationId());
        Map<Long, List<GrabfoodItemMappingViewDTO>> groupedByCategory = grabfoodItemMappingViewDTOS.stream()
                .collect(Collectors.groupingBy(GrabfoodItemMappingViewDTO::getGrabItemGroupId));

        SystemMain systemMainItemIdListDiffZone = systemMainService.findSystemPropertyName("GRABFOOD_ITEM_ID_LIST_DIFF_ZONE").orElse(null);

        SystemMain systemMainLocationIdListDiffZone = systemMainService.findSystemPropertyName("GRABFOOD_LOCATION_ID_LIST_DIFF_ZONE").orElse(null);

        SystemMain systemMainImgLink = systemMainService.findSystemPropertyName("IMG_LINK").orElse(null);

        return groupedByCategory.values().stream().map(itemMappings -> {
                    GrabfoodItemMappingViewDTO categoryRepresentative = itemMappings.get(0);
                    CategoryDTO category = new CategoryDTO();
                    category.setId(categoryRepresentative.getGrabCategoryId());
                    category.setName(categoryRepresentative.getGrabCategory());
                    category.setAvailableStatus(AvailableStatus.AVAILABLE.name());
                    category.setSellingTimeID("SELLING-TIME-01");
                    category.setSequence(categoryRepresentative.getGrabCategorySequence());

                    List<ItemDTO> items = getItemDTOS(itemMappings, currency, systemMainItemIdListDiffZone, systemMainLocationIdListDiffZone, systemMainImgLink, location);
    ...
    private List<ItemDTO> getItemDTOS(List<GrabfoodItemMappingViewDTO> items, CurrencyDTO currency, SystemMain systemMainItemIdListDiffZone, SystemMain systemMainLocationIdListDiffZone, SystemMain systemMainImgLink, Location location) {
        ...
        List<Long> itemMasterIds = items.stream()
                .map(GrabfoodItemMappingViewDTO::getItemMasterId)
                .collect(Collectors.toList());

        List<ItemMaster> itemMasters = itemMasterService.findItemMastersByItemMasterIds(itemMasterIds);
        ...
        return items.stream().map(itemMapping -> {
            ...
            if(itemMaster != null){
                ...
                item.setDescription(itemMapping.getDescription());
                ...
                if(item.getPrice() != null && item.getPrice() <= 0){
                    ...
                    item.setAvailableStatus(AvailableStatus.AVAILABLE.name());
                }
    ...
    public ResponseEntity<?> menuSyncState(MenuSyncStateRequestDTO data) {
        ...
        grabMenuSyncStatus.setProduct(Product.GRAB_FOOD.name());
